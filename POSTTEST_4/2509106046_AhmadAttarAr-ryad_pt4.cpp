...
Tiket dequeue(){
    ...
    cout << "Rute : " << data.rute << endl;

    return data;
}

void tampilAntrian(Tiket *head){
    if(head == NULL){
        cout << "Antrian kosong\n";
        return;
    }
    ...
    while(current != NULL){
        cout << current->nama << " - " << current->rute << endl;
...
void pop(){
    ...
    cout << "Hapus transaksi: " << hapus->nama << endl;
...
void peekStack(){
    if(top == NULL){
        ...
        cout << "Transaksi terakhir: " << top->nama << endl;
    }
}
